 ...
 * 
 */
function DataSet$1(data, options) {
    Event.bind(this)();
...
DataSet$1.prototype = Object.create(Event.prototype);

/**
 * Add data.
 */
DataSet$1.prototype.add = function (data, senderId) {
...
DataSet$1.prototype.reset = function () {
 ...
 * get data.
 */
DataSet$1.prototype.get = function (args) {
    args = args || {};
 ...
 * set data.
 */
DataSet$1.prototype.set = function (data) {
    this._set(data);
 ...
 * set data.
 */
DataSet$1.prototype._set = function (data) {
    this.clear();
    this.add(data);
};
 ...
 * clear data.
 */
DataSet$1.prototype.clear = function (args) {
    this._data = []; // map with data indexed by id
};
 ...
 * remove data.
 */
DataSet$1.prototype.remove = function (args) {};

/**
 * update data.
 */
DataSet$1.prototype.update = function (cbk, condition) {
 ...
 * transfer coordinate.
 */
DataSet$1.prototype.transferCoordinate = function (data, transferFn, fromColumn, toColumnName) {
...
DataSet$1.prototype.initGeometry = function (transferFn) {
 ...
 * 获取当前列的最大值
 */
DataSet$1.prototype.getMax = function (columnName) {
    var data = this._data;
 ...
 * 获取当前列的总和
 */
DataSet$1.prototype.getSum = function (columnName) {
    var data = this._data;
 ...
 * 获取当前列的最小值
 */
DataSet$1.prototype.getMin = function (columnName) {
    var data = this._data;
 ...
 * 获取去重的数据
 */
DataSet$1.prototype.getUnique = function (columnName) {
    var data = this._data;
 ...
 * 获取平均值
 */
DataSet$1.prototype.getAverage = function (columnName) {
    var data = this._data;
 ...
 */
DataSet$1.prototype.sort = function (columnName) {
 ...
 */
DataSet$1.prototype.getPage = function (page, pageSize) {
 ...
 * 获取数据总数
 */
DataSet$1.prototype.getTotal = function () {
 ...
 */
DataSet$1.prototype.groupBy = function (columnName) {
    var groups = {};
 ...
 */
DataSet$1.prototype.find = function (predicate) {
 ...
 * @param {function} predicate - 查找条件函数
 */
DataSet$1.prototype.findAll = function (predicate) {
 ...
 */
DataSet$1.prototype.map = function (mapper) {
 ...
 */
DataSet$1.prototype.filter = function (filter) {
...
var pathSimple = {
     ...
     * 绘制图片标记
     */
    drawImageMarker: function drawImageMarker(context, data, options) {
        var coordinates = data.geometry._coordinates || data.geometry.coordinates;
        var x = coordinates[0];
        var y = coordinates[1];
     ...
     * 增强版多边形绘制，支持渐变填充
     */
    drawPolygonEnhanced: function drawPolygonEnhanced(context, coordinates, options) {
        context.beginPath();
...
var drawSimple = {
     ...
     * 增强版绘制函数，支持更多自定义选项
     */
    drawEnhanced: function drawEnhanced(context, dataSet, options) {
        var data = dataSet instanceof DataSet$1 ? dataSet.get() : dataSet;
...
Category.prototype.getDefaultColors = function () {
    return ['rgba(255, 255, 0, 0.8)', 'rgba(253, 98, 104, 0.8)', 'rgba(255, 146, 149, 0.8)', 'rgba(255, 241, 193, 0.8)', 'rgba(110, 176, 253, 0.8)', 'rgba(52, 139, 251, 0.8)', 'rgba(17, 102, 252, 0.8)', 'rgba(0, 176, 104, 0.8)', 'rgba(128, 128, 128, 0.8)', 'rgba(255, 0, 255, 0.8)'];
};
...
Choropleth.prototype.addRange = function (range) {
    ...
    // 重新排序以确保区间顺序正确
    this.splitList.sort(function (a, b) {
...
var drawCluster = {
     ...
     * @param {*} context 画布上下文
     */
    drawClusterPoint: function drawClusterPoint(item, options, context) {
            ...
            stacks[url].push(function (x, y, iconWidth, iconHeight) {
                return function (img) {
                    ...
                    if (options.iconOptions.show) {
                        ...
                        // 动态字体大小（根据聚合点数量调整）
                        var _pointCount2 = item.properties && item.properties.point_count || 0;
                        // 修改为图标大小的一半
                        var _fontSize = options.iconOptions.fontSize || Math.floor(iconWidth / 6);
                        context.font = _fontSize + 'px Arial'; // 使用 normal 字体样式

                        // 绘制文字（完全居中）
                        var _text = _pointCount2.toString();
                        if (_text) {
                            var _textX = x + iconWidth / 2; // 图标水平中心
                            // const textY = y + iconHeight / 2; // 图标垂直中心
                            // context.fillText(text, textX, textY);
                            // 为了更精确的垂直居中，需要考虑字体度量
                            // 获取字体的度量信息
                            var _metrics = context.measureText(_text);
                            // 计算文字的实际高度
                            var _fontHeight = _metrics.actualBoundingBoxAscent + _metrics.actualBoundingBoxDescent;
                            // 调整Y坐标以实现真正的垂直居中
                            var _textY = y + iconHeight / 2 + (_metrics.actualBoundingBoxAscent - _fontHeight / 2);

                            context.fillText(_text, _textX, _textY);
                            // context.strokeStyle = 'red';
                            // context.beginPath();
                            // context.moveTo(x + iconWidth / 2, y);
                            // context.lineTo(x + iconWidth / 2, y + iconHeight);
...
var drawIcon = {
    draw: function draw(context, dataSet, options) {
        ...
        var _loop = function _loop() {
            ...
            if (item.geometry) {
                // 如果是聚类点且配置了图标选项
                if (item.properties && item.properties.cluster) {
                    _this.drawClusterIcon(item, options, context);
                } else {
     ...
     * @param {*} context 画布上下文
     */
    drawClusterIcon: function drawClusterIcon(item, options, context) {
        var _this2 = this;
        ...
        if (iconOptions.show !== false && (iconOptions.url || item.icon)) {
            ...
            if (iconUrl) {
                ...
                if (img) {
                    ...
                    stacks$1[url].push(function (img) {
                        if (img === 'error') {
                            _this2.drawDefaultPoint(context, x, y, item, options);
                        } else {
                            _this2.drawImageWithText(context, img, x, y, item, options, iconOptions);
                        }
                    });
                }
                return;
     ...
     * 绘制默认点
     */
    drawDefaultPoint: function drawDefaultPoint(context, x, y, item, options) {
        context.beginPath();
        context.arc(x, y, options.size || 5, 0, Math.PI * 2);
     ...
     * 绘制带文字的图片
     */
    drawImageWithText: function drawImageWithText(context, img, x, y, item, options, iconOptions) {
        context.save();
     ...
     * 绘制默认的聚类圆点
     */
    drawDefaultClusterPoint: function drawDefaultClusterPoint(context, x, y, item, options) {
        context.save();
...
var BaseLayer = function () {
    ...
    createClass(BaseLayer, [{
        ...
        key: 'getDataInBounds',
        value: function getDataInBounds(bounds) {
        ...
        key: 'hoverEvent',
        value: function hoverEvent(pixel, e) {
        ...
        value: function doubleClickEvent(pixel, e) {
        ...
        value: function updateOptions(options) {
            ...
            if (options.max !== undefined || options.min !== undefined || options.gradient !== undefined || options.splitList !== undefined) {
...
var Layer = function (_BaseLayer) {
    ...
    function Layer(map, dataSet, options) {
        ...
        _this.doubleClickEvent = _this.doubleClickEvent.bind(_this);
    ...
    createClass(Layer, [{
        ...
        key: 'doubleClickEvent',
        value: function doubleClickEvent(e) {
            var pixel = e.pixel;
            get(Layer.prototype.__proto__ || Object.getPrototypeOf(Layer.prototype), 'doubleClickEvent', this).call(this, pixel, e);
        }
    }, {
        key: 'bindEvent',
        ...
        key: 'getDataInBounds',
        value: function getDataInBounds() {
            var bounds = this.map.getBounds();
            var ne = bounds.getNorthEast();
            var sw = bounds.getSouthWest();
            ...
            if (this.options.draw === 'cluster' && (!this.options.maxClusterZoom || this.options.maxClusterZoom >= zoom)) {
                ...
                // 简单过滤，实际应该使用更精确的方法
                return this.dataSet.get().filter(function (item) {
                    if (item.geometry && item.geometry.coordinates) {
                        var coords = item.geometry.coordinates;
        ...
        key: 'getLayerInfo',
        value: function getLayerInfo() {
            return {
                zoom: this.getZoom(),
                visible: this.canvasLayer && this.canvasLayer.canvas ? this.canvasLayer.canvas.style.display !== 'none' : true,
    ...
    createClass(MapVRenderer, [{
        ...
        value: function _canvasUpdate(t) {
            ...
            if (this.canvasLayer && !this.stopAniamation) {
                ...
                this.drawContext(n, new DataSet$1(c), this.options, h), this.options.updateCallback && this.options.updateCallback(t);
            }
        }
    }, {
        key: "updateData",
