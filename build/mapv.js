(function (global, factory) {
    ...
    var drawSimple = {
         ...
         * @param {DataSet|Array} dataSet - 数据集实例或数据数组
         * @param {Object} options - 绘制选项
         */
        draw: function draw(context, dataSet, options) {

            // 如果dataSet是DataSet实例，则获取其数据，否则直接使用
            var data = dataSet instanceof DataSet ? dataSet.get() : dataSet;
    ...
    */
    ...
    var drawCluster = {
         ...
         * @param {DataSet|Array} dataSet - 数据集实例或数组
         * @param {Object} options - 绘制选项
         */
        draw: function draw(context, dataSet, options) {
            // 保存当前画布状态
            ...
            if (options.avoid) {
                ...
                for (var i = 0, len = data.length; i < len; i++) {
                    ...
                    if (!hasOverlay(rects, rect)) {
                        ...
                        px = px + textWidth / 2;
                        py = py + size / 2;
                        context.fillStyle = data[i].fillStyle || options.fillStyle || 'black';
